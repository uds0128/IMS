...
<body>
   ...
   $sql = "SELECT max(VendorId) FROM bTblVendorMst";
...
if(isset($_POST['submit'])){
  ...
  error_reporting(E_ERROR | E_WARNING | E_PARSE);
         ...
         $myque = "SELECT VendorId FROM btblvendormst WHERE VendorName = '$name' AND MobileNo = '$vmno' AND GSTNo = '$vgst' AND Email = '$vemail'";
         ...
         elseif ($myque3 != 0){
            $myque4 = "SELECT RecStatus FROM btblvendormst WHERE VendorId=$myque3";
            $myque5 = mysqli_query($conn,$myque4);
            $row = mysqli_fetch_row($myque5);
            $myque6=$row[0];
            if ($myque6==0) {
              $myque7 = "UPDATE btblvendormst SET RecStatus=1 WHERE VendorId=$myque3";
            ...
            $que = "INSERT INTO btblvendormst (VendorName,MobileNo,Email,GSTNo,Address,CreatedDate,ModifiedDate,RecStatus) VALUES('$name','$vmno','$vemail','$vgst','$vaddress',NOW(),NOW(),1)";
