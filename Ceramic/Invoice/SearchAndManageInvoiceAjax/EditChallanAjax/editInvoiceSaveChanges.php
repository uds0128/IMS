<?php
    ...
    //    $oldotherqty       = $invoicedata[$i]['oldotherqty'];
      ...
      //  $updateotherqty    = $otherqty - $oldotherqty;
       ...
       $editInvoiceDetails = "UPDATE invoicedetails SET Quantity = Quantity + ({$updatebillingqty}), SellingPrice = {$sellingprice} WHERE InvoiceId = {$invoiceid} and StockId= {$stockid}";
        ...
        $updateStockDetails = "UPDATE stockdetails SET BillingQty = BillingQty - ({$updatebillingqty}) WHERE StockId = {$stockid}";
    ...
    $editIninvoiceMst = "UPDATE invoicemst SET Discount = '{$discount}', TransportationCost = '{$transportationcost}', ModifiedDate = now(), TotalAmount = {$total} WHERE InvoiceId = {$invoiceid}";
