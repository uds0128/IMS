   ...
   <script>
      function checkRadio(radio) {
         if (radio.id === "customerName") {
            document.getElementById("hsnc").innerHTML = "<input type='text' name='cname' style='margin-bottom: 20px; margin-left: 550px;' class='form-control' id='name' placeholder='Enter Customer Name'>";
         }
         else if (radio.id === "vendorName") {
            document.getElementById("hsnc").innerHTML = "<input type='text' name='vname' style='margin-bottom: 20px; margin-left: 700px;' class='form-control' id='name' placeholder='Enter Vendor Name'>";
         }
         else if (radio.id === "date") {
            document.getElementById("hsnc").innerHTML = "<div class='row col-md-12' style='margin-bottom: 20px; margin-left: 450px;'><div class='col-md-6'><label for=''>From Date:</label><input type='date' class='form-control get-date' id='date1' name='date1'></div><div class='col-md-6'><label for=''>To Date:</label><input type='date' class='form-control get-date' id='date2' value='<?php echo $today; ?>' name='date2'></div></div>";
         }
         else if (radio.id === "challanno") {
            document.getElementById("hsnc").innerHTML = "<input type='number' name='challanno' style='margin-bottom: 20px; margin-left: 850px;' class='form-control col-md-4' id='name' placeholder='Enter Challan Number'>";
         }
         else if (radio.id === "InwardId") {
            document.getElementById("hsnc").innerHTML = "<input type='number' name='InwardId' style='margin-bottom: 20px; margin-left: 1000px;' class='form-control col-md-4' id='name' placeholder='Enter Inward ID'>";
         }
      }
   </script>
</head>
            ...
            <table class="table" id="data_table">
               ...
               <tbody>
                  ...
                  <?php
                     ...
                     {
                        $query= "SELECT Discount,TransportCost,ExtraCost,PaymentDate,ChallanNo,CustomerName,TotalAmount,tblinwardpayment.AmountPaid,tblinwardpayment.AmountPending,PaymentID FROM ((tblinwardpayment
                                 INNER JOIN challanmst ON tblinwardpayment.ChallanId = challanmst.ChallanId)
                                 INNER JOIN tblcustomermst ON challanmst.CustomerId = tblcustomermst.CustomerId) WHERE tblinwardpayment.RecStatus=1 and tblinwardpayment.StockMstSysId=0 UNION SELECT TotalDiscount,Transport_extracost,Transport_extracost,PaymentDate,tblinwardpayment.InwardId,VendorName,TotalAmount,tblinwardpayment.AmountPaid,tblinwardpayment.AmountPending,PaymentID FROM ((tblinwardpayment 
                     ...
                     {
                        $query= "SELECT Discount,TransportCost,ExtraCost,PaymentDate,tblinwardpayment.InwardId,VendorName,TotalAmount,tblinwardpayment.AmountPaid,tblinwardpayment.AmountPending,PaymentID FROM ((tblinwardpayment
                                 INNER JOIN tblinwardbillmst ON tblinwardpayment.InwardId = tblinwardbillmst.InwardId)
                     ...
                     else if ($_POST['cname']=='' and $_POST['vname']=='' and $_POST['date1'] and $_POST['date2']=='' and $_POST['challanno']=='' and $_POST['InwardId']=='') 
                     {
                        $query= "SELECT Discount,TransportCost,ExtraCost,PaymentDate,ChallanNo,CustomerName,TotalAmount,tblinwardpayment.AmountPaid,tblinwardpayment.AmountPending,PaymentID FROM ((tblinwardpayment
                                 INNER JOIN challanmst ON tblinwardpayment.ChallanId = challanmst.ChallanId)
                                 INNER JOIN tblcustomermst ON challanmst.CustomerId = tblcustomermst.CustomerId) WHERE PaymentDate LIKE '$date1'  and tblinwardpayment.RecStatus=1 and tblinwardpayment.StockMstSysId=0 ORDER BY PaymentDate DESC UNION SELECT PaymentDate,tblinwardpayment.InwardId,VendorName,TotalAmount,tblinwardpayment.AmountPending,tblinwardpayment.AmountPaid,PaymentID FROM ((tblinwardpayment 
                                 INNER JOIN tblinwardbillmst ON tblinwardpayment.InwardId = tblinwardbillmst.InwardId)
                     ...
                     else if ($_POST['cname']=='' and $_POST['vname']=='' and $_POST['date1'] and $_POST['date2'] and $_POST['challanno']=='' and $_POST['InwardId']=='') 
                     {
                        $query= "SELECT Discount,TransportCost,ExtraCost, PaymentDate,ChallanNo,CustomerName,TotalAmount,tblinwardpayment.AmountPaid,tblinwardpayment.AmountPending,PaymentID FROM ((tblinwardpayment
                                 INNER JOIN challanmst ON tblinwardpayment.ChallanId = challanmst.ChallanId)
                                 INNER JOIN tblcustomermst ON challanmst.CustomerId = tblcustomermst.CustomerId) WHERE PaymentDate BETWEEN '$date1' AND '$date2' and tblinwardpayment.RecStatus=1 and tblinwardpayment.StockMstSysId=0 UNION SELECT PaymentDate,tblinwardpayment.InwardId,VendorName,TotalAmount,tblinwardpayment.AmountPaid,tblinwardpayment.AmountPending,PaymentID FROM ((tblinwardpayment 
                                 INNER JOIN tblinwardbillmst ON tblinwardpayment.InwardId = tblinwardbillmst.InwardId)
